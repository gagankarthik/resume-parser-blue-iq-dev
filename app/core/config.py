...
class Settings(BaseSettings):
    ...
    # an unbounded 100-page fax would OOM/timeout the Lambda. Resumes are a few
    ...
    # so a seed (with a pinned model snapshot) makes identical resumes extract
    ...
    openai_seed: int = 7
    # Output-token ceiling. A dense resume (many roles, each with bullets +
    ...
    # resumes at the cost of more LLM calls; the single-shot parser remains the
    ...
    # sparse one-role resumes (< ~1K chars) stay on the fast single-shot path.
    ...
    # the API score as confidence. ON by default so every resume gets a city_id
    ...
    # de-duplicated and capped per resume. Uses the same platform key as the other
    # Gig endpoints.
    gig_cities_api_url: str = "https://api.gighealth.com/api/v1/external/cities"
    enable_city_api_match: bool = True
    # Upper bound on distinct city lookups per resume (protects latency + quota).
