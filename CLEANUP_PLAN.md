...
discard the catalogs, the tuned prompts, and 478 tests' worth of hard-won résumé edge cases, and
...
`README.md:225` and `docs/ARCHITECTURE.md:20` state résumé content is never stored. The
**`feedback` table persists original + corrected parsed JSON — full résumé PII — for 90 days.**
...
(agents) means **the same résumé can be classified differently depending on which path parsed
